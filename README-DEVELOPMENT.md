...
We showcase a Pulumi-owned provider based on an upstream provider named `terraform-provider-transip`.  Substitute appropriate values below for your use case.
...
* `provider/` contains the Go code used to create the provider as well as generate the SDKs in the various languages that Pulumi supports.
  * `provider/cmd/pulumi-tfgen-transip` generates the Pulumi resource schema (`schema.json`), based on the Terraform provider's resources.
  * `provider/cmd/pulumi-resource-transip` generates the SDKs in all supported languages from the schema, placing them in the `sdk/` folder.
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource transip_something not found in provider map; skipping
    warning: resource transip_something_else not found in provider map; skipping
    warning: data source transip_something not found in provider map; skipping
    warning: data source transip_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "transip_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "transip_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "transip_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "transip_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. **Add provider configuration overrides (not typically needed):** Pulumi's Terraform bridge automatically detects configuration options for the upstream provider.  However, in rare cases these settings may need to be overridden, e.g. if we want to change an environment variable default from `API_KEY` to `transip_API_KEY`.  Examples of common uses cases:
    ...
    "apikey": {
        Default: &tfbridge.DefaultInfo{
            EnvVars: []string{"transip_API_KEY"},
        },
    ```
...
1. Copy the `pulumi-resource-transip` binary generated by `make provider` and place it in your `$PATH` (`$GOPATH/bin` is a convenient choice), e.g.:

    ```bash
    cp bin/pulumi-resource-transip $GOPATH/bin
    ```
...
1. Create a new Pulumi program in the `examples/` directory, e.g.:
    ...
    npm install
    yarn link @maienm/pulumi-transip
    ```
